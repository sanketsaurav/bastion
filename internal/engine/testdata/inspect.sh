...
sg=missing; sudo -n test -f /etc/sudoers.d/bastion-user-alias 2>/dev/null && sg=granted
if e=$(getent passwd dev); then f ualias "$(printf %s "$e" | cut -d: -f3)" "$(id -u)" "$sg"; else f ualias absent "$(id -u)" "$sg"; fi
t="$HOME"/.tmux.conf
if [ -e "$t" ]; then
...
if [ -e "$t" ]; then
  if [ -r "$t" ]; then sha=$(sha256sum <"$t" | cut -d' ' -f1); else sha=unreadable; fi
  f file L2V0Yy9hcHQvYXB0LmNvbmYuZC81MWJhc3Rpb24tdW5hdHRlbmRlZC1yZWJvb3Q= present "$sha" "$(stat -c %a "$t" 2>/dev/null || echo '?')"
else f file L2V0Yy9hcHQvYXB0LmNvbmYuZC81MWJhc3Rpb24tdW5hdHRlbmRlZC1yZWJvb3Q= absent; fi
if [ -e "$t.bastion-backup" ]; then f bak L2V0Yy9hcHQvYXB0LmNvbmYuZC81MWJhc3Rpb24tdW5hdHRlbmRlZC1yZWJvb3Q= present; else f bak L2V0Yy9hcHQvYXB0LmNvbmYuZC81MWJhc3Rpb24tdW5hdHRlbmRlZC1yZWJvb3Q= absent; fi
m=$(cat /var/lib/bastion/state/testbox/files/c9dc08f25a1d0538.json 2>/dev/null || true); [ -n "$m" ] && f marker file c9dc08f25a1d0538 "$(eb "$m")"
